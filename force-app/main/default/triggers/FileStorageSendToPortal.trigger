trigger FileStorageSendToPortal on fra__File_Storage__c (after insert, after update, after delete) {
    ...
    if (Trigger.isAfter && Trigger.isUpdate) {
        ...
        for (fra__File_Storage__c newFs : Trigger.new) {
            ...
            Boolean titleChange        = oldFs.Name                       != newFs.Name;
            Boolean documentTypeChange = oldFs.Document_Type__c           != newFs.Document_Type__c;
            ...
            if (
                ...
                ||
                (isStillEnabled && (dateChange || titleChange
                    || documentTypeChange || sourceTypeChange || categoryChange || publicURl))
            ) {
        ...
        if (!aiSourceFileIds.isEmpty()) {
            System.debug('aiSourceFileIds to cascade: ' + aiSourceFileIds);
