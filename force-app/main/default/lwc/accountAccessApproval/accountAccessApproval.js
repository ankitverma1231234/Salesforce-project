...
import validateToken from '@salesforce/apex/AccountAccessApprovalController.validateToken';
import processApproval from '@salesforce/apex/AccountAccessApprovalController.processApproval';
...
export default class AccountAccessApproval extends LightningElement {
    ...
    connectedCallback() {
        ...
        const urlAction = (params.get('action') || 'approve').toLowerCase();
        this.action = urlAction === 'reject' ? 'reject' : 'approve';
        ...
        validateToken({ token: this.token })
            .then((info) => {
                this.requestorName = info.requestorName || 'A family member';
                this.memberFirstName = info.memberFirstName || '';
    ...
    get approveVariant() { return this.action === 'reject' ? 'neutral' : 'brand'; }
    get rejectVariant()  { return this.action === 'reject' ? 'destructive' : 'neutral'; }
}